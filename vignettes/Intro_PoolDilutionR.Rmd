...
Errors are weighted by a combination of data quality and explanatory power (see `cost_function()`) and predictions are made using the package function `ap_prediction()`. 
See Under the hood for more information.
=======
To solve for gross methane production and consumption for this sample, we call pdr_optimize(), which uses optim() to minimize the error in the observed vs predicted pool size and isotopic composition. Errors are weighted by a  combination of data quality and explanatory power (*ie*., cost_fuction()) and predictions are made using ap_prediction(). See *Under the hood.* for more information.\
...
consumption; and `k`, the consumption rate constant.
The first two are taken from default values for methane as provided in the 
package's `pdr_fractionation` dataset, while the latter is estimated from the data.
