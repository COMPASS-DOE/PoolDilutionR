...
```{r Solve for P and k}
results <- pdr_optimize(time = OneSample_dat$time_days, # time as a vector
...
Now we show a graphical summary of the results from running these samples through `pdr_optimize_tidy()` with the same arguments as shown for a single sample above.  
...
The equations in `ap_prediction()` come from the following equations in [von Fischer and Hedin 2002](https://doi.org/10.1029/2001GB001448). 
\
\
...
### Cost Function  
  ...
  The `cost_function()` provides feedback to `ap_prediction()` on the quality of each iteration of fitted rates and/or fractionation constants. The sum of errors are weighted by the standard deviation of the observations, as well as a scaling factor, ($N$).  
\
\
  **Equation 14**:
