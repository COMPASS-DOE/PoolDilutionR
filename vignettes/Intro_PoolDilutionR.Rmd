...
pdr_optimize(time = OneSample_dat$time_days, # time as a vector
             ...
             ap_prec = 0.001, # instrument precision for atom percent, as standard deviation
)
```

Notice that `pdr_optimize` calculated or looked up three parameters that we didn't provide:
`frac_P`, the fractionation rate of production; `frac_k`, the fractionation rate of
consumption; and `k0`, the initial value for the first order rate constant for consumption.
The first two are taken from default values for methane as provided in the 
package's `pdr_fractionation` dataset, while the latter is estimated from the data, using the slope of the pool size of heavy isotope over time (`n_slope = -11.635`).

`pdr_optimize` returns a list with the following entries:

* `par` - the final optimized values of (in this case) production (`P`) and consumption (`k`), as calculated by `optim()`
...
Now we show a graphical summary of the results from running these samples through `pdr_optimize()` with the same arguments as shown for single sample above.  
 
```{r lapply(PoolDilutionR)}
samples <- split(Morris2023, Morris2023$id)
#lapply(samples, pdr_optimize_tidy)
...
```
\
Poor fits can result from a variety of issues, including instrument error and incorrect assumptions regarding fractionation rates. 
...
```{r}
#Sample 52 above has poor fit for atom%13C

Sample52_dat <- subset(Morris2023, subset = id == "52")

original_fit<- pdr_optimize_tidy(time = Sample52_dat$time_days,
                  m = Sample52_dat$cal12CH4ml + Sample52_dat$cal13CH4ml, 
                  n = Sample52_dat$cal13CH4ml,
...
P_originalfit <- original_fit[original_fit$par == "P",]$value
k_originalfit <- original_fit[original_fit$par == "k",]$value

new_fit<- pdr_optimize_tidy(time = Sample52_dat$time_days,
                  m = Sample52_dat$cal12CH4ml + Sample52_dat$cal13CH4ml, 
                  n = Sample52_dat$cal13CH4ml,
                  P = P_originalfit,
                  k = k_originalfit,
...
#dataframe with original fit (optimizing P and k)
y_og <- ap_prediction(time = Sample52_dat$time_days,
                          m0 = Sample52_dat$cal12CH4ml[1] + Sample52_dat$cal13CH4ml[1],
                          n0 = Sample52_dat$cal13CH4ml[1],
                          P = P_originalfit,
                          k = P_originalfit,
                          pool = "CH4")
dat_og <- cbind(Sample52_dat, y_og)
dat_og$fit <- "Original"

fracP_newfit <- new_fit[new_fit$par == "frac_P",]$value
frack_newfit <- new_fit[new_fit$par == "frac_k",]$value

#dataframe with new fit (optimizing P_frac and k_frac)
y_new <- ap_prediction(time = Sample52_dat$time_days,
                          m0 = Sample52_dat$cal12CH4ml[1] + Sample52_dat$cal13CH4ml[1],
                          n0 = Sample52_dat$cal13CH4ml[1],
                          P = P_originalfit,
                          k = P_originalfit,
...
dat_new <- cbind(Sample52_dat, y_new)
dat_new$fit <- "New"

comparison <- rbind(dat_new, dat_og)

#graph two fits
ggplot(data = comparison) +
  geom_point(aes(time_days, AP_pred, color = "Prediction")) +
  geom_point(aes(time_days, AP_obs, color = "Observation")) +
  scale_color_manual(breaks = c("Prediction", "Observation"),
                     values = c('Observation' = 'blue',
                                'Prediction' = 'red')) +
  ylab("Atom Percent 13C") +
  facet_grid(.~fit)
```

http://127.0.0.1:30403/graphics/2e962c38-c005-4557-934f-612fee28735e.png
