...
Here we see the relative goodness of fit for final optimized P and k values (colored line) in terms of predicted isotopic signature (Atom% 13C) and pool size (Total Methane) for six samples. Grey lines represent values tested on the way to the final optimized fit.  
\
...
**Equation 9**; which tracks the change in the heavy isotopologue over time in terms of consumption and it's fractionation constants.  
...
Equation 10; which gives the atom percent content to the pool for each time step in terms of gross rates and their fractionation constants.  
...
$$ AP_t = \frac {n_t}{m_t} + AP_p $$
  ...
  $n_t$ is the pool size of the heavy isotopologue at time *t*, as above.  
  $m_t$ is the total pool size at time *t*.  
  
Equations 9 and 10 are combined in Equation 11;  
